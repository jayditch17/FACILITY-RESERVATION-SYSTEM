...
if(isset($_POST["userID"]) && !empty($_POST["userID"])){
    ...
    if(empty($firstName_err) && empty($lastName_err) && empty($dept_err) && empty($email_err) && empty($pass_err)){
        ...
        $sql = "UPDATE account_fac SET first_name=?, last_name=?, deptFac=?, facEmail=?, facPass=? WHERE userID=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssi", $param_fname, $param_lName, $param_dept, $param_email, $param_pass, $param_userID);
